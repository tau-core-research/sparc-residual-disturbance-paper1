...
We present a residual-blind phenomenological audit of low-acceleration SPARC rotation-curve residual scores. Coherence labels were assigned from source-backed external evidence, frozen before residual audit, and compared against a fixed rms-log endpoint. In the quality-selected sample, the primary C-minus-A median difference for the fixed low-acceleration residual score is 0.08427, with one-sided permutation p = 0.00100 and bootstrap 95% CI [0.04481, 0.15990]. The signal remains positive in leave-one-out, radius common-support, radius-matched, distance-matched, and distance-stratified controls. Effect-size checks show moderate A/C separation for the fixed score (AUC = 0.771) and for MOND/RAR-like baselines (AUC = 0.721-0.731), but not for the Newtonian baryonic baseline (AUC = 0.506). Residual distance/radius imbalance and observability systematics remain important caveats. A later conditional predictive-increment audit found no stable projection-specific increment in the same frozen A/C packet, and a source-frozen seven-galaxy EDGE-CALIFA morphology-proxy stress test failed its directional endpoint. These later results leave the marginal association intact but prevent interpreting it as conditional projection-specific information or independent external validation.

## 1. Motivation

The central question is whether external, residual-blind structural disturbance is marginally associated with a fixed SPARC residual-scatter endpoint. If the endpoint is sensitive to real astrophysical disturbance rather than only fitting noise or post-hoc grouping, externally disturbed galaxies should show larger scatter than externally regular disks.
...
Two downstream audits narrow this interpretation without changing any Paper 1 statistic. First, a retrospective Paper 3 fixed-model conditional predictive-increment audit used the same frozen 45-galaxy A/C sample, repeated stratified five-fold cross-fitting with 20 repetitions, endpoint-residual-free observability/baryonic covariates, and residual structure common to MOND/RAR-like baselines. The unique projection contrast supplied no stable held-out proper-score increment under either declared baseline and any of the three declared regularization values. At the primary O7B7, C=0.3 setting, the AUC change was -0.01890756, the log-loss improvement was -0.03994321, and the Brier-score improvement was -0.01188683; negative improvement means worse performance. This is an operational conditional predictive-increment result, not a literal conditional-mutual-information estimate and not a theorem that all additional rotation information is absent.

Second, a source-frozen seven-galaxy EDGE-CALIFA rotation-morphology proxy endpoint returned mean D=-0.058679293503004035, exact one-sided p=0.6015625, median D=+0.11020194395988532, and 4/7 positive galaxy-level values. It therefore failed its directional prevalidation gate. This is an external morphology-proxy stress test, not an independent Tau-specific test and not a direct replication of the Paper 1 A/C endpoint.

The subsequent public-data ladder also remains below claim-raising status. A prospective LITTLE THINGS replay transfers one frozen scale-tail family to 14 galaxies and 313 points, but gives only a 0.101 km/s mean RMSE advantage over TPG/v6, is 0.649 km/s worse than MOND, and cannot test multi-family attribution. In PHANGS, a three-galaxy morphology-orthogonal low-order CO--Halpha statistic is consistent with zero (p=0.8082); a separate two-galaxy m=2 statistic is numerically non-null but its m=1 control is also non-null and its source-label integrity audit fails. A higher-dimensional four-galaxy PHANGS confirmatory packet stops at its predeclared azimuth-sector occupancy gate and releases no score. These results constrain different proxy endpoints rather than directly repeating the Paper 1 A/C statistic.

The claim boundary after these audits is consequently an unconditional or marginal within-SPARC association. Paper 1 does not establish conditional projection-specific information, an independently replicated morphology law, or a physical Tau mechanism. The additional public-data routes strengthen the falsification record, not the physical claim.

These failures must also be read at the resolution actually tested. Each audit freezes a finite morphology representation Phi_k, not a source-complete parent body. A terminal law can descend through Phi_k only if two body states with the same Phi_k value always have the same terminal image. Failure therefore constrains the declared representation and descent, not every possible richer morphology. This distinction is not a license for post-hoc rescue: once terminal vector y is known, appending y to a source span makes its projected residual zero by construction. Any richer representation must be built from source-only information, frozen, and tested on a new untouched packet.
...
The seven-galaxy EDGE-CALIFA morphology-proxy stress test does not satisfy this Phase II requirement: it used a different proxy endpoint, failed its directional gate, and was not an independent Tau-specific test. A claim-raising replication still requires a new source-frozen matched-tracer cohort with the nuisance and comparator structure fixed before endpoint inspection.
...
The full reproducibility package, including the frozen labels, external evidence table, baseline-score comparisons, control summaries, figures, and regeneration scripts, is archived at doi:10.5281/zenodo.20285859. The analysis can be regenerated with the commands listed in Section 13.
...
The result indicates a reproducible marginal residual-disturbance association in the audited SPARC sample. A natural interpretation is that C-class systems contain more non-equilibrium or non-circular motion, increasing scatter for smooth axisymmetric models. Because distance/radius imbalance remains, MOND/RAR-like baselines show compatible disturbance sensitivity, the later conditional projection-contrast audit is negative, and the seven-galaxy EDGE-CALIFA morphology-proxy stress test failed, this is not standalone proof of the projection model. The supported statement is narrower: the frozen residual score is unconditionally associated with residual-blind external labels in this SPARC sample; conditional projection-specific information and independent external replication remain unestablished.
