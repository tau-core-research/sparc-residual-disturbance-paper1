...
The result should therefore be read as a residual-disturbance audit, not as a unique model-selection result. A natural alternative interpretation is that the C class contains more non-equilibrium or non-circular systems, for which any smooth axisymmetric rotation-curve score would be expected to perform worse.
